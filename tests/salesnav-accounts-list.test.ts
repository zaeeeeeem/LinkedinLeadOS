...
import {
  accountsPagedRunner,
  createSalesNavAccountsCapability,
  projectProvedPages,
  type SalesNavAccountsDeps,
} from "../src/capabilities/salesnav.accounts.list/index.js";
import {
  createAccountsSource,
  selectAccountPage,
  type AccountsCursor,
  type AccountsSourceRuntime,
} from "../src/capabilities/salesnav.accounts.list/source.js";
import { runPaged } from "../src/core/paged/index.js";
import { describe, expect, it, vi } from "vitest";

function body(page: number, company = page, tracking = `tracking-${page}`): string {
  const count = 25;
  return JSON.stringify({
    paging: { total: 75, count, start: (page - 1) * count },
    elements: [{
      entityUrn: `urn:li:fs_salesCompany:${company}`,
      companyName: `Company ${company}`,
      industry: "Software",
      employeeCountRange: "B-D",
      employeeDisplayCount: 42,
      description: "Description",
      trackingId: tracking,
      listCount: 0,
      saved: false,
      spotlightBadges: [],
      companyPictureDisplayImage: { artifacts: [] },
    }],
  });
}
...
function archived(file: string, seq = 0): ArchivedCapture {
  return {
    seq, id: file, file, path: `/tmp/${file}`, shapeHash: "shape", url: "https://www.linkedin.com/sales-api/salesApiAccountSearch",
    status: 200, capturedAt: "2026-08-11T00:00:00.000Z", bytes: 1,
  };
}

function capture(page: number, company = page, seq = page, tracking?: string): Capture {
  const entry = archived(`${page}.json.gz`, seq);
  const text = body(page, company, tracking);
  return {
    seq,
    pattern: "salesapi-account-search",
    patterns: ["salesapi-account-search", "linkedin-data"],
    requestId: String(page),
    url: `https://www.linkedin.com/sales-api/salesApiAccountSearch?sessionId=session-a&page=${page}`,
    status: 200,
    body: text,
    bytes: text.length,
    archived: entry,
...
describe("Sales Navigator account page evidence", () => {
  it("selects the named endpoint by the body's page offset", () => {
    ...
    expect(got.parsed.rows[0]).toMatchObject({ page: 2, position: 1, company_urn: "urn:li:fs_salesCompany:2" });
    expect(got.parsed.rows[0]).not.toHaveProperty("person_urn");
  });

  it("fails a clicked page whose body offset did not advance", () => {
    expect(() => selectAccountPage([capture(1)], 2, [])).toThrowError(expect.objectContaining({ code: "PAGE_DID_NOT_ADVANCE" }));
  });

  it("classifies a changed paging shape as parse drift", () => {
    const changed = capture(1);
    changed.body = JSON.stringify({ elements: [] });
    expect(() => selectAccountPage([changed], 1, [])).toThrowError(expect.objectContaining({
  ...
  it("refuses two bodies at one offset when their stable identities disagree", () => {
    expect(() => selectAccountPage([capture(2, 20, 1), capture(2, 21, 2)], 2, []))
      .toThrowError(expect.objectContaining({ code: "SALESNAV_PAGE_AMBIGUOUS" }));
  });

  it("keys page identity on entityUrn, never the per-execution trackingId", () => {
    const got = selectAccountPage([
      capture(2, 20, 1, "execution-a"),
      capture(2, 20, 2, "execution-b"),
    ], 2, []);
    expect(got.parsed.rows[0]?.company_urn).toBe("urn:li:fs_salesCompany:20");
    expect(got.captures).toHaveLength(2);
  });
  ...
  it("returns the tap's exact archive ids to the paged loop", async () => {
    ...
    expect(runtime.gate).toHaveBeenCalledWith(expect.objectContaining({
      state: expect.objectContaining({ paged: { preserved: true }, salesnav_accounts: expect.any(Object) }),
    }));
  });
  ...
  it("checks the prior session before clicking and trusts the arrived body offset", async () => {
    ...
    const runtime = {
      ...
      read: vi.fn(async () => ({ passes: 3, scrolled: 300, layout: { settled: true } })),
      click: vi.fn(async () => {
        currentUrl = "https://www.linkedin.com/sales/search/company?sessionId=session-a&page=2";
  ...
  it("refuses a changed session before any pager click", async () => {
    ...
    const browser = {
      ...
      tab: {
        ...
        currentUrl: async () => "https://www.linkedin.com/sales/search/company?sessionId=session-b&page=1",
      },
      cursor: {},
    } as unknown as BrowserBundle;
    const source = createAccountsSource({
      ...
      sessionUrns: () => [],
    }, { click } as unknown as AccountsSourceRuntime);
    const prior: AccountsCursor = { kind: "salesnav-accounts/v1", session_id: "session-a", page: 1, start: 0, count: 25, arrival: "navigate" };
...
describe("archive-proved storage projection", () => {
  it("re-reads every proved page, including pages absent from this session's loaded list", async () => {
    ...
    expect(got.rows).toEqual([
      { search_id: "run-a", page: 1, position: 1, company_urn: "urn:li:fs_salesCompany:1", run_ref: "run-a" },
      { search_id: "run-a", page: 2, position: 1, company_urn: "urn:li:fs_salesCompany:2", run_ref: "run-a" },
    ]);
  });
...
function composition(failSecond = false) {
  ...
  const deps: SalesNavAccountsDeps = {
  ...
  const ctx = {
    run: {
      runId: "run-a", resumed: false, args: { url: "https://www.linkedin.com/sales/search/company" }, dir: "/tmp/run-a",
      log: vi.fn(), elapsedMs: () => 10, artifacts: () => ({ events: "runs/a/events.ndjson", raw: "runs/a/raw/" }),
      lastCheckpoint: () => null, checkpoint: vi.fn(),
    },
    args: { url: "https://www.linkedin.com/sales/search/company", pages: 2, limit: 50 },
  ...
  return { capability: createSalesNavAccountsCapability(deps), deps, ctx, insertResults };
}

describe("salesnav.accounts.list composition", () => {
  it("composes the exact Task 35 paged runner", () => {
    expect(accountsPagedRunner).toBe(runPaged);
  });
  ...
  it("writes each proved page as its own bounded store batch", async () => {
    ...
    expect(deps.ensureSearch).toHaveBeenCalledWith(expect.objectContaining({
      search_id: "run-a",
      kind: "sn_accounts",
      filter_url: "https://www.linkedin.com/sales/search/company",
    }), expect.anything());
