...
--
-- DO NOT EDIT THIS FILE ONCE IT HAS BEEN APPLIED ANYWHERE. Add a new migration instead.
-- Every statement here is `if not exists`, which makes editing silently ineffective: an
-- added column in an existing `create table if not exists` block is ignored outright, with
-- no error. `npm run db:verify` cannot catch that either, because it resets the database
-- first and therefore only ever exercises a fresh apply (D99).
...
create table if not exists public.raw_captures (
  ...
  -- No `on delete cascade`: the gzipped bodies live on disk under runs/<run_id>/raw/ and
  -- are the durable copy (D2). Cascading would delete the index into them and leave files
  -- nobody can find, so deleting a run that has captures is made to fail instead (D98).
...
-- Exposure
--
-- RLS on with no policies, plus explicit grants to service_role and explicit revokes from
-- anon and authenticated. The store client holds the service role key, which bypasses RLS.
-- The agent reads bulk data over a direct Postgres connection (D4), the superuser path,
-- unaffected by either.
--
-- RLS alone is NOT the deny-by-default it looks like. It filters rows for
-- select/insert/update/delete and says nothing about table-level privileges, and Supabase's
-- bootstrap default ACL for role `postgres` on this schema hands anon and authenticated
-- TRUNCATE, REFERENCES, TRIGGER and MAINTAIN on every table a migration creates. A grant
-- only ever adds, so nothing below the revokes would have removed them: anon could truncate
-- all 13 tables (D97). PostgREST issues no DDL, so that was not reachable over the Data API
-- today — it was a standing privilege contradicting the stated model.
--
-- Assertions on the grants therefore have to run against the database, not against this
-- text: a privilege this file never wrote is invisible to a test that reads this file.
-- `npm run db:verify` queries information_schema.role_table_grants for them.
-- ---------------------------------------------------------------------------
...
-- `on all tables` is a snapshot of what exists at this moment, so the same rules are set as
-- defaults for whatever later migrations create. These target the current role (postgres),
