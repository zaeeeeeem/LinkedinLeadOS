...
import { MAX_SALESNAV_ROWS_PER_PAGE, parseSalesNavLeads, type SalesNavLeadsParseResult } from "./parse.js";
import { createLeadsSource, leadsFingerprint, type LeadsCursor, type LeadsPageData, type LeadsSourceOptions } from "./source.js";
...
const args = z.object({
  ...
  limit: z.coerce.number().int().min(1).max(MAX_PAGES_PER_RUN * MAX_SALESNAV_ROWS_PER_PAGE).default(DEFAULT_LIMIT),
...
type LeadsArgs = z.infer<typeof args>;
type LeadsContext = CapabilityContext<LeadsArgs, true>;

export type SalesNavLeadsDeps = {
  store(): StoreClient;
  ...
  source(options: LeadsSourceOptions): PagedSource;
};

const defaults: SalesNavLeadsDeps = {
  store: getStore,
...
function targetFor(ctx: LeadsContext): SalesNavTarget {
  let raw = ctx.args.url;
  if (raw === undefined && ctx.run.resumed) raw = readPagedState(ctx.run)?.plan;
  if (raw === undefined) {
    throw refusal("SALESNAV_URL_REQUIRED", "a fresh salesnav.leads.list run requires --url=<Sales Navigator people search>");
  }
  const target = normalizeSalesNavUrl(raw);
  if (target.vertical !== "people") {
    throw refusal("SALESNAV_VERTICAL_MISMATCH", "salesnav.leads.list accepts only /sales/search/people targets");
  }
...
function finishShape(ctx: LeadsContext, status: "ok" | "error", exitCode: number): RunRecordFinish {
  return {
    status,
...
export async function projectProvedPages(o: {
  ...
  for (const page of o.pages) {
    const parsed: SalesNavLeadsParseResult[] = [];
    for (const id of page.archive_ids) {
      const body = await o.readText(id);
      const result = parseSalesNavLeads(body, { sessionUrns: o.sessionUrns });
      if (result.paging?.page === page.page) parsed.push(result);
    }
    if (parsed.length === 0) {
      throw refusal(
        "SALESNAV_PROVED_PAGE_UNREADABLE",
        "a page corroborated by archive filenames no longer yields its expected lead-search body",
        `page=${page.page} archive_ids=${page.archive_ids.length}`,
      );
    }
    const fingerprints = new Set(parsed.map((result) => leadsFingerprint(result.rows)));
...
function clickData(pages: readonly CompletedPage[]) {
  return pages.flatMap((page) => {
    const cursor = page.cursor as Partial<LeadsCursor> | undefined;
...
function loadedWarnings(outcome: PagedRunOutcome): Warning[] {
  return outcome.loaded.flatMap(({ load }) => {
    const data = load.data as Partial<LeadsPageData> | undefined;
...
export function createSalesNavLeadsCapability(deps: SalesNavLeadsDeps = defaults) {
  return defineCapability({
    name: "salesnav.leads.list",
    risk: "read-metered",
    summary: "Read a bounded, paged Sales Navigator lead search with archive-proved resume and provenance storage.",
    args,
    needsBrowser: true,
    cost: (input) => {
    ...
    run: async (ctx) => {
      ...
      try {
        ...
        return {
          ...
          next: stored === null
            ? `read the archived lead-search bodies under ${ctx.run.artifacts().raw}`
            : `select search_id, page, position, person_urn, run_ref from search_results where search_id = '${ctx.run.runId}' order by page, position`,
        };
      } catch (cause) {
...
export const capability = createSalesNavLeadsCapability();
